...
本项目同时使用本地目录和 Git 分支两个概念，必须明确区分：

- 本地目录：
  - 主项目目录：`F:\0_GPT_道路设计原型功能项目`，是用户日常查看、加载和稳定继续开发的基准目录。
  - worktree 目录：`.worktrees/<分支名>`，用于某个任务的隔离开发。
- Git 分支：
  - `main` 分支：Git 主线分支。
  - worktree 分支 / 功能分支：与某个 worktree 目录对应的隔离开发分支，通常使用 `codex/<任务名>`。

主项目目录当前通常检出 `main` 分支，但“主项目目录”和“main 分支”不是同一个概念；worktree 目录和 worktree 分支也不是同一个概念。后续描述必须尽量写清是“本地目录”还是“Git 分支”。

## Worktree 隔离开发规则

本项目经常使用 `.worktrees/<分支名>` 做隔离开发。worktree 的目的就是隔离任务：在 worktree 目录中开发时，默认只修改该 worktree 目录，并在对应 worktree 分支中提交和推送。

禁止在 worktree 任务收尾时自动把正式代码和文档同步回主项目目录。主项目目录应保持稳定，不被多个并行 worktree 任务反复覆盖。只有在用户明确确认某个 worktree 任务已经完成、需要合入或需要主项目目录可见副本时，才执行合并、快进、挑拣提交或按指定范围同步到主项目目录。

确认合入或同步到主项目目录时，默认可纳入的正式源码和文档范围包括：
...
约定：worktree 中形成或更新的正式代码与文档，以 worktree 分支提交并推送到 Git 的版本作为正式留档。主项目目录不再承担“每个 worktree 收尾都必须保留最新副本”的职责。若用户要求后续“基于主项目目录更新”，应先确认要采用哪个分支或提交，再通过 Git 合入、快进、挑拣提交，或按用户指定范围同步到主项目目录。

## Worktree 构建产物同步规则

在 `.worktrees/<分支名>` 内构建时，ARX / 托管 DLL 默认输出到该 worktree 的 `artifacts/` 目录。构建产物也保持 worktree 隔离，默认不复制到主项目目录。

只有在用户明确要求从主项目目录直接加载、查看、调试或分发，或确认该 worktree 成果要进入主项目目录时，才把最新构建产物同步到主项目目录 `F:\0_GPT_道路设计原型功能项目`。需要同步时的默认位置：
...
这条规则只补充构建产物的复制位置；代码和文档合入或同步必须遵守上面的“Worktree 隔离开发规则”。不要把构建缓存、IDE 状态或临时文件混入源代码同步。
